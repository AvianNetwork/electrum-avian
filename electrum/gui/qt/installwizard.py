...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def select_storage(
        ...
        button_create_new.clicked.connect(
            lambda: name_e.setText(get_new_wallet_name(wallet_folder))
    ...
    @wizard_dialog
    def add_xpub_dialog(
        self, title, message, is_valid, run_next, allow_multi=False, show_wif_help=False
    ):
    ...
    def derivation_and_script_type_gui_specific_dialog(
        ...
        *,
        ...
        clayout = ChoicesLayout(
            message1, c_titles, on_choice_click, checked_index=default_choice_idx
        )
        if not hide_choices:
            vbox.addLayout(clayout.layout())
