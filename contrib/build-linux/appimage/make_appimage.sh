...
APPIMAGE="$DISTDIR/electrum-avian-$VERSION-x86_64.AppImage"
...
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-avian-pypi.txt"
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-avian-git.txt"
...
cp "$PROJECT_ROOT/electrum-avian.desktop" "$APPDIR/electrum-avian.desktop"
