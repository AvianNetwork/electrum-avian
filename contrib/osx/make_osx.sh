...
GIT_REPO=https://github.com/AvianNetwork/electrum-avian
...
info "Installing dependencies sepcific to avian..."
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-avian-pypi.txt \
    || fail "Could not install dependencies from pypi"
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-avian-git.txt \
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-avian-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-avian-${VERSION}.dmg"
